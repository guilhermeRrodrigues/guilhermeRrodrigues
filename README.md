<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:6C5CE7,50:8E7CFF,100:00D9FF&height=200&section=header&text=Guilherme%20Rodrigues&fontSize=54&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Full%20Stack%20Developer%20%E2%80%A2%20Founder%20%E2%80%A2%20Linux%20e%20DevOps&descAlignY=56&descSize=17" width="100%"/>

<a href="https://github.com/guilhermeRrodrigues">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=23&duration=3200&pause=800&color=8E7CFF&center=true&vCenter=true&width=680&lines=Founder+%40+ADJ+Tech;Criador+do+cwords.fun;Full+Stack+%E2%80%A2+Next+%E2%80%A2+Nest+%E2%80%A2+Python+%E2%80%A2+Docker;Especialista+em+Linux+e+Infraestrutura;Medalha+de+Ouro+OBR+%E2%80%A2+Nacional+OBI" alt="Typing SVG" />
</a>

<br/>

<a href="https://cwords.fun"><img src="https://img.shields.io/badge/cwords.fun-00D9FF?style=for-the-badge&logo=steam&logoColor=white" /></a>
<a href="https://adjtech.com.br"><img src="https://img.shields.io/badge/ADJ_Tech-6C5CE7?style=for-the-badge&logo=googlechrome&logoColor=white" /></a>
<a href="mailto:contato@adjtech.com.br"><img src="https://img.shields.io/badge/Contato-EA4335?style=for-the-badge&logo=gmail&logoColor=white" /></a>

</div>

<br/>

## 🧠 Sobre mim

```yaml
nome:      Guilherme Rodrigues
funcao:    Full Stack Developer & Founder
empresa:   ADJ Tech — Tecnologia em Soluções Inteligentes
produto:   cwords.fun
foco:      ["Arquitetura de Software", "DevOps", "IA Aplicada", "Infraestrutura"]
stack:     [TypeScript, Python, Next.js, NestJS, Docker, Linux]
filosofia: "Linguagem é sintaxe. O que importa é resolver o problema."
```

- 🚀 **Founder** da [**ADJ Tech**](https://adjtech.com.br) — desenvolvimento web, software sob medida, DevOps e cibersegurança
- 🎮 **Criador** do [**cwords.fun**](https://cwords.fun) — jogo multiplayer de encadeamento de palavras
- 🐳 **Docker & DevOps** — containers, orquestração e pipelines CI/CD em produção
- 🐧 **Linux** — administração de servidores, redes e infraestrutura self-hosted
- 🤖 Robótica competitiva, sistemas embarcados e IA aplicada
- 🌐 Aprendo linguagem nova conforme o projeto pede — a ferramenta se adapta ao problema

---

## 🏆 Conquistas

| Competição | Resultado |
|:---|:---|
| **OBR** — Olimpíada Brasileira de Robótica, Modalidade Teórica | 🥇 **Medalha de Ouro** |
| **OBR** — Modalidade Artística | **3x participante** |
| **OBI** — Olimpíada Brasileira de Informática (Programação) | 🏆 **Vencedor — fase Regional** |
| **OBI** — Fase Nacional | **Classificado** |

> 💡 O código do projeto de robótica está aberto em [**OBR-TechTitans2025**](https://github.com/guilhermeRrodrigues/OBR-TechTitans2025).

---

## 🛠️ Stack

<div align="center">

**Uso todo dia**

<img src="https://skillicons.dev/icons?i=ts,python,nextjs,nestjs,react,nodejs,docker,linux,postgres,git&theme=dark" />

**Também trabalho com**

<img src="https://skillicons.dev/icons?i=dart,flutter,go,cpp,bash,redis,nginx,prisma,tailwind,githubactions&theme=dark" />

</div>

---

## 🚀 Produtos

<table>
<tr>
<td width="50%" valign="top">

### 🎯 cwords.fun
**Jogo multiplayer de encadeamento de palavras.**

Meu maior projeto. Partidas em tempo real, matchmaking e salas simultâneas.

`Next.js` · `WebSocket` · `Docker` · `PostgreSQL`

🔗 **[cwords.fun](https://cwords.fun)**

</td>
<td width="50%" valign="top">

### 🏢 ADJ Tech
**Tecnologia em Soluções Inteligentes.**

Desenvolvimento web, software sob medida, consultoria de TI, DevOps e cibersegurança.

`NestJS` · `Next.js` · `Docker` · `CI/CD`

🔗 **[adjtech.com.br](https://adjtech.com.br)**

</td>
</tr>
</table>

---

## 💻 Open Source

<table>
<tr>
<td width="33%" valign="top">

### ✂️ [clipgen](https://github.com/guilhermeRrodrigues/clipgen)
**Do link ao post em um clique.**

Cole a URL de um vídeo do YouTube: a IA identifica os melhores momentos, corta, edita e legenda automaticamente — pronto pra publicar em todas as redes.

`Python` · `Whisper` · `LLM` · `FFmpeg`

</td>
<td width="33%" valign="top">

### 🎮 [playIA](https://github.com/guilhermeRrodrigues/playia)
**IA que aprende te vendo jogar.**

Observa a gameplay, aprende os padrões e passa a jogar sozinha — melhorando a cada partida.

`Python` · `Machine Learning` · `Computer Vision`

</td>
<td width="33%" valign="top">

### 🕵️ [secret-browser](https://github.com/guilhermeRrodrigues/secret-browser)
**Navegador focado em privacidade.**

Navegação sem rastreamento, com botão de saída rápida para fechar a aplicação instantaneamente.

`Dart` · `Flutter` · `Privacy`

</td>
</tr>
</table>

---

<div align="center">

### 💬 Vamos construir algo?

<a href="https://adjtech.com.br"><img src="https://img.shields.io/badge/Fale_comigo-6C5CE7?style=for-the-badge&logo=maildotru&logoColor=white" /></a>
<a href="https://cwords.fun"><img src="https://img.shields.io/badge/Jogue_cwords-00D9FF?style=for-the-badge&logo=steam&logoColor=white" /></a>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00D9FF,50:8E7CFF,100:6C5CE7&height=120&section=footer" width="100%"/>

</div>
